            ...
            <div className="flex gap-4 scroll-x pb-1">
              {catArtists.map(a => (
                <button key={a.name} onClick={() => loadArtist(a)}
                  ...
                  <div className={`relative w-[72px] h-[72px] sm:w-20 sm:h-20 rounded-full overflow-hidden transition-all duration-300 shadow-lg ${
                    activeArtist === a.name
                      ? 'ring-2 ring-rose-400 shadow-xl shadow-rose-500/20 scale-110'
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${activeArtist === a.name ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-xl ${activeArtist === a.name ? 'bg-rose-500' : 'bg-white/90'}`}>
                        <Play size={12} className={activeArtist === a.name ? 'text-white ml-0.5' : 'text-black ml-0.5'} fill={activeArtist === a.name ? 'white' : 'black'} />
                      </div>
                    </div>
                  </div>
                  <p className={`text-[10px] sm:text-[11px] font-medium text-center w-[72px] sm:w-20 truncate transition-colors duration-200 ${
                    activeArtist === a.name ? 'text-rose-400' : 'text-white/50 group-hover:text-white/80'
                  }`}>{a.name}</p>
                </button>
              ))}
            </div>

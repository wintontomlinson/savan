  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const profiles = await Promise.all(
        Object.keys(artwork)
          .slice(0, 8)
          .map((name) => searchArtists(name, 1).then((result) => result[0]).catch(() => null)),
      );
    ...
    <div className="home-page pb-5 pt-5 sm:pt-7">
      ...
      {status === 'error' && (
        <div className="home-error">
          ...
          <h2>Music is taking a moment.</h2>
          <p>Check your connection, then let Savan build the next set for you.</p>
          <button onClick={refresh} className="home-primary-button press"><RefreshCw size={14} /> Try again</button>
        </div>
      )}
        ...
        <div className="home-mood-grid">
          {MOODS.map((mood, index) => {
            ...
            return (
              <button
                key={mood.id}
                onClick={() => startMood(mood)}
                aria-pressed={activeMood === mood.id}
                className={`home-mood-card group bg-gradient-to-br ${mood.color}`}
              >
...
function HomeStageSkeleton() {
  return (
    <div className="home-stage home-stage-skeleton">
      <div className="home-stage-copy space-y-3"><div className="skeleton h-3 w-28" /><div className="skeleton h-10 w-4/5" /><div className="skeleton h-4 w-2/3" /><div className="flex gap-2"><div className="skeleton h-11 w-28 rounded-full" /><div className="skeleton h-11 w-24 rounded-full" /></div></div>
      <div className="skeleton h-[200px] w-[200px] rounded-[28px]" />
    </div>
  );
}
...
function ShelfSkeleton() {
  return (
    <div className="mb-10">
      ...
      <div className="flex gap-4 overflow-hidden">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="w-[170px] shrink-0"><div className="skeleton aspect-square rounded-2xl" /><div className="skeleton mt-3 h-3 w-3/4" /></div>)}</div>
    </div>
  );
}

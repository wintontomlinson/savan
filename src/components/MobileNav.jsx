...
export default function MobileNav() {
  return (
    <div className="bg-[#0a0a0a]/95 backdrop-blur-2xl border-t border-white/[0.05]">
          ...
          <NavLink key={i.to} to={i.to} className={({ isActive }) =>
            ...
            {({ isActive }) => (
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${
                isActive ? 'bg-rose-500/10 scale-110' : 'hover:bg-white/[0.04]'
              }`}>
                <i.icon size={20} strokeWidth={isActive ? 2.2 : 1.5} className={`transition-colors duration-300 ${isActive ? 'text-rose-400' : ''}`} />
              </div>
            )}
          </NavLink>
        ))}

  ...
  return (
    <aside className="hidden md:flex flex-col w-[72px] lg:w-[240px] h-full bg-[#030303] border-r border-white/[0.04] fixed left-0 top-0 z-20">
      ...
      <div className="px-4 lg:px-5 py-5 flex items-center gap-2.5">
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-fuchsia-500 to-violet-500 flex items-center justify-center shadow-lg shadow-fuchsia-500/25 relative overflow-hidden animate-breathe">
      ...
      <nav className="flex-1 px-2 lg:px-3 mt-2 space-y-0.5">
        {nav.map(item => (
          <NavLink key={item.to} to={item.to} className={({ isActive }) =>
            ...
            {({ isActive }) => (
              <>
                {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-[20px] bg-gradient-to-b from-fuchsia-400 to-violet-500 rounded-r-full" />}
                <item.icon size={20} strokeWidth={isActive ? 2.2 : 1.5} className={`transition-all duration-300 ${isActive ? 'text-fuchsia-400' : 'group-hover:text-white/80'}`} />
        ...
        <div className="px-2 lg:px-3 pb-4 mt-auto">
          <div onClick={() => setExpanded(true)} className="p-2.5 lg:p-3 bg-gradient-to-br from-white/[0.04] to-white/[0.01] rounded-2xl border border-white/[0.05] cursor-pointer hover:bg-white/[0.06] transition-all duration-300 group">
            <div className="flex items-center gap-2.5">
              <div className={`w-10 h-10 rounded-xl overflow-hidden shrink-0 ring-1 ring-white/[0.08] transition-all duration-300 ${isPlaying ? 'shadow-lg shadow-fuchsia-500/20 group-hover:scale-105' : ''}`}>
                <img src={currentSong.thumbnail} alt="" className="w-full h-full object-cover" />
              </div>
              <div className="hidden lg:block min-w-0 flex-1">
                <p className="text-[11px] font-semibold text-white truncate group-hover:text-fuchsia-300 transition-colors">{currentSong.title}</p>
            ...
            {isPlaying && (
              <div className="hidden lg:flex items-center gap-1.5 mt-2.5 px-0.5">
                <Disc3 size={11} className="text-fuchsia-400 animate-[spin_3s_linear_infinite]" />
                <span className="text-[9px] text-fuchsia-400/80 font-medium">Playing</span>
              </div>
            )}
          </div>
        </div>

...
export default function SongRow({ song, index, songList = [] }) {
  ...
  return (
    <div className={`group flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-3 transition-all duration-200 cursor-pointer ${
      isActive ? 'bg-fuchsia-500/[0.06]' : 'hover:bg-white/[0.03] active:bg-white/[0.06]'
    }`}
      ...
      <div className="w-6 shrink-0 flex justify-center">
        {isActive && isPlaying ? <Equalizer /> : (
          <>
            <span className={`text-[12px] tabular-nums group-hover:hidden ${isActive ? 'text-fuchsia-400 font-semibold' : 'text-white/20'}`}>{index + 1}</span>
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-[13px] font-medium truncate leading-tight transition-colors duration-200 ${isActive ? 'text-fuchsia-400' : 'text-white group-hover:text-white'}`}>{song.title}</p>
      ...
      <button onClick={e => { e.stopPropagation(); toggleLike(song.id); }}
        className={`p-1.5 shrink-0 transition-all duration-200 active:scale-90 rounded-full ${
          liked ? 'text-fuchsia-400' : 'text-white/15 sm:opacity-0 sm:group-hover:opacity-100 hover:text-white/50'
        }`}>

...
export default function SongCard({ song, songList, subtitle }) {
  ...
  return (
    <button
      onClick={onPlay}
      className="group w-[144px] shrink-0 text-left sm:w-[164px]"
      aria-label={`Play ${song.title} by ${song.artist}`}
    >
      <div
        className={`art relative mb-2.5 aspect-square overflow-hidden rounded-xl transition-shadow duration-300 ${
      ...
      >
        ...
        <span className="absolute bottom-2.5 right-2.5 flex h-10 w-10 translate-y-2 items-center justify-center rounded-full bg-accent text-white opacity-0 shadow-xl shadow-black/50 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">

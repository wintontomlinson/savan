...
import{Home,Compass,Library,Settings,Music}from'lucide-react';
import{useAuth}from'../context/AuthContext';

const links=[{to:'/',icon:Home,label:'Home'},{to:'/explore',icon:Compass,label:'Explore'},{to:'/library',icon:Library,label:'Library'}];
  ...
  return(
    <aside className="hidden md:flex flex-col w-[72px] lg:w-[240px] h-full bg-[#1A1A1A] border-r border-[#383838] fixed left-0 top-0 z-30 pt-16">
      <div className="px-4 lg:px-6 py-4 flex items-center gap-2">
        ...
        <span className="hidden lg:block text-white font-bold text-lg">Savan</span>
      </div>
      ...
      <div className="px-3 lg:px-4 py-4 border-t border-[#383838] flex items-center gap-3">
        ...
        <div className="hidden lg:block min-w-0"><p className="text-sm text-white truncate">{user.name}</p></div>
      </div>
    </aside>
  );
}
